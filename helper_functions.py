...
def load_sam_model(model_type):
    ...
    # Determine device
    if torch.cuda.is_available(): # Check for CUDA first (not applicable for most Macs)
        device = torch.device("cuda")
        st.write("Using device: CUDA")
    elif torch.backends.mps.is_available():
        # MPS (Apple Silicon GPU) is available.
        # However, the segment_anything library currently has an issue where it attempts to
        # use float64 data types for coordinates on MPS, which is not supported, leading to errors.
        # Forcing CPU for now to ensure the application runs.
        # Performance will be slower than a fully MPS-compatible execution.
        device = torch.device("cpu")
        st.write("Using device: CPU (MPS available but using CPU due to library compatibility with float64 data types on MPS)")
    else: # Fallback to CPU if neither CUDA nor MPS is available
        device = torch.device("cpu")
        st.write("Using device: CPU")

    # st.write(f"Using device: {device}") # Original debug print, replaced by more specific ones above
